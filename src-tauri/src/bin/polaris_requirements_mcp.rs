//! Requirements MCP Binary Entry Point
//!
//! Usage: polaris-requirements-mcp <config_dir> [workspace_path]
//!        polaris-requirements-mcp <workspace_path>  (legacy format, uses default config_dir)
//!
//! Arguments:
...
fn main_impl() -> Result<()> {
    ...
    if args.len() < 2 {
        return Err(AppError::ValidationError(
            "缺少参数。用法：polaris-requirements-mcp <config_dir> [workspace_path]".to_string(),
        ));
    }
...
fn parse_args(args: &[String]) -> Result<(String, Option<&str>)> {
    match args.len() {
        2 => {
            ...
            // If the path exists as a directory and contains .polaris
        ...
        _ => {
            Err(AppError::ValidationError(
                "参数过多。用法：polaris-requirements-mcp <config_dir> [workspace_path]".to_string(),
            ))
        }
    }
}
